function h=error_ellipse_superimposed(varargin)
...
if r==3 & c==3
  ...
  h1=plot3(x0+k*x,y0+k*y,1+k*z,prop.style);hold on
  %h1=plot3(x0+k*x,y0+k*y,1+k*z,prop.style, 'Color', [prop.style, 0.2]);hold on
  set(h1, 'LineWidth', 1);
  [y,z,x] = getpoints(Cyz,prop.clip);
  h2=plot3(0+k*x,y0+k*y,z0+k*z,prop.style);hold on
  %h2=plot3(0+k*x,y0+k*y,z0+k*z,prop.style, 'Color', [prop.style, 0.2]);hold on
  set(h2, 'LineWidth', 1);
  [z,x,y] = getpoints(Czx,prop.clip);
  %h3=plot3(x0+k*x,0+k*y,z0+k*z,prop.style);hold on
