function [Amat, angles_tp, angles_tp_sub] = forwardmodelDoorwayCam1(px,py,h,Ntp,Ntp_sub,rho_hid)
%FORWARDMODELDOORWAYCAM1 computes the contribution of light sources in the visible
...
for ii = 1:length(denom_t)
    if ii == length(denom_t)
        [visMat] = dwvisibility([rho_hid angles_tp_sub(ii,:)],meas_p);
    end
    radial_falloff = (rho_hid.^2)./( (rho_hid./denom_t(ii) - meas_p(:,1)).^2 + ...
        (rho_hid./denom_t(ii).*tan(TH(ii)) - meas_p(:,2)).^2 + ...
        (rho_hid.*tan(PH(ii))./denom_t(ii) - meas_p(:,3)).^2 );
    [visMat] = dwvisibility([rho_hid angles_tp_sub(ii,:)],meas_p);
...
[TH, PH] = meshgrid(linspace(0.000,pi/2-0.001,Nt), linspace(min_angle_psi, pi/2-0.001,Np));
angles_tp = [TH(:),PH(:)];
...
function [visMat] = dwvisibility(s_rtp,measPlane)
...
visibility_v = (( atan(measPlane(:,2)./measPlane(:,1)) - (theta_s)<0));
visibility_h = (( atan(measPlane(:,3)./measPlane(:,1)) - ( psi_s )<0));
